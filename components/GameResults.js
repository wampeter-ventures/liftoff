...
function GameResults({
    ...
    useEffect(() => {
        if (victoryLevel === 0 && onWolfStart) {
            const t = setTimeout(() => setShowWolfHint(true), 2000);
            return () => clearTimeout(t);
        }
        return undefined;
    }, [victoryLevel, onWolfStart]);
        ...
        <div className="game-results mt-4 px-4 relative">
            ...
            >
                ...
                <CardFooter className="p-4 sm:p-5 pt-6 sm:pt-8 border-t dark:border-slate-700/50 flex flex-col gap-3">
                    {showWolfHint && !wolfOutcome && (
                        <div className="wolf-hint text-center">
                            <p className="text-sm text-slate-600 dark:text-slate-300 mb-1">Signs of Life from a distant Star...</p>
                            <Button
                                size="lg"
                                onClick={onWolfStart}
