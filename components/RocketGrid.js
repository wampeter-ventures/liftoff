...
function RocketGrid({
    ...
    const getPlanetName = (rows, boosters) => {
        ...
        if (rows >= 4) return 'Saturn';
        if (rows >= 3) return 'Jupiter';
        if (rows >= 2) return 'Ceres';
        if (rows >= 1) return 'Mars';
        return 'Earth';
    };
    ...
    const headerText = React.useMemo(() => {
        ...
        return `Mission: ${getPlanetName(completedBodyRows.length, boosterCount)}`;
    }, [boosterRowLocked, rocketComplete, completedBodyRows, boosterCount]);
