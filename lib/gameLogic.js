...
const GameLogic = {
    ...
    isValidPlacement(
        ...
        if (dieValue >= 1 && dieValue <= 5) {
            if (row === 6) return false; // Row 6 is boosters-only
    ...
    adventureDestinations: [
        ...
        { name: 'Eris', rows: 5, boosters: 6, bonus: true },
        { name: 'Wolf 1061', rows: 5, boosters: 12 },
    ],

    getAdventureDestination(rocketGrid, boosterRowLocked) {
        if (!boosterRowLocked) return null;
        const bodyRows = GameLogic.getCompletedRows(rocketGrid).filter(r => r < 6).length;
        const boosters = Object.values(rocketGrid).filter(d => d && d.value === 6).length;
        for (let i = GameLogic.adventureDestinations.length - 1; i >= 0; i--) {
            const dest = GameLogic.adventureDestinations[i];
            if (bodyRows >= dest.rows && boosters >= dest.boosters) {
                return { index: i, ...dest };
            }
        }
        return null;
