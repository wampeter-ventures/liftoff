...
const GameLogic = {
    ...
    calculateVictoryLevel(
        ...
        const completedBodyRows = GameLogic.getCompletedRows(rocketGrid).filter(
            (r) => r <= 5,
        ).length;
        if (completedBodyRows >= 5) return 5; // Neptune
        if (completedBodyRows >= 4) return 4; // Saturn
        if (completedBodyRows >= 3) return 3; // Jupiter
        if (completedBodyRows >= 2) return 2; // Mars
        if (completedBodyRows >= 1) return 1; // Moon
        return 0;
    },
